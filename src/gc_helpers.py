...
def send_command(connection: MAVConnection, command: int, param1=0.0, param2=0.0, param3=0.0, param4=0.0, param5=0.0, param6=0.0, param7=0.0, target_system=1, target_component=mavlink.MAV_COMP_ID_ONBOARD_COMPUTER, retries:int = 3, retry_timeout_s: float=1.0):
    ...
    for attempt in range(retries):
        ...
        if ack_event.wait(timeout=retry_timeout_s):
            connection.sub_manager.unsubscribe(mavlink.MAVLink_command_ack_message.msgname, on_ack)
            return ack_result == mavlink.MAV_RESULT_ACCEPTED or ack_result == mavlink.MAV_RESULT_IN_PROGRESS
...
def configure_messages(connection: MAVConnection):
    ...
    send_command(connection=connection, command=mavlink.MAV_CMD_SET_MESSAGE_INTERVAL, target_component=0, param1=mavlink.MAVLINK_MSG_ID_GPS_RAW_INT, param2=two_hz_us)
